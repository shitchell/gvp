...
- **`mkdirSync(recursive)` can hang forever under `/proc`.** procfs answers `mkdir(2)` with `ENOENT` even when the parent exists, and Node's recursive mkdir treats `ENOENT` as "create the parent and retry" — an infinite loop. Not hypothetical: it hung the suite three times during Task 9 before being isolated. `writeFileAtomic` inherits it, so a `GVP_REGISTRY_ROOT` under procfs turns D57's "never throws" into "never returns", which is strictly worse than throwing. Fixing it needs a decision about validating the registry root, so it is recorded rather than patched.
- **A third YAML walker.** `findYamlFilesRecursive` (`src/cli/commands/import.ts:725`) is byte-identical to the now-shared `findYamlFiles` modulo its name. Collapsing it is one line and squarely P11, but `import.ts` is outside Task 9's file list.
